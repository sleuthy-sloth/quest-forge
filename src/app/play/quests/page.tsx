...
/** Earliest timestamp that counts as "within the current period" for a recurrence type. */
function getPeriodStart(rec: Recurrence): Date {
  ...
  switch (rec) {
    ...
    case 'weekly': {
      ...
      mon.setDate(today.getDate() - ((now.getDay() + 6) % 7)) // roll back to Monday
      return mon
    }
              ...
              return (
                <article
                  ...
                  style={{
                    ...
                    border:          `1px solid ${stateGlow.border}`,
                    borderRadius:    3,
                    overflow:        'hidden',
                    boxShadow:       stateGlow.shadow,
                    transition:      'border-color 0.2s, box-shadow 0.2s',
                  }}
                >
                  {/* Difficulty colour bar (left edge) */}
                  <div
                    aria-hidden="true"
                    style={{
                      position:   'absolute',
                  ...
                  {/* Parchment radial tint */}
                  <div
                    aria-hidden="true"
                    style={{
                      position:       'absolute',
                      inset:           0,
                      background:     `radial-gradient(ellipse 180% 60% at 50% 0%, ${stateGlow.tint}, transparent 65%)`,
                      pointerEvents:  'none',
                    }}
                  />
                  ...
                  <div style={{ padding: '1rem 1rem 1rem 1.25rem', position: 'relative' }}>
                    ...
                    {/* ── Header row: gem + title + state badge ──────── */}
                    ...
                    >
                      {/* Flavor text — the "quest" in fantasy language */}
                      <p
                        style={{
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                        ...
                        <span
                          style={{
                            ...
                            fontSize:       '0.42rem',
                            imageRendering: 'pixelated',
                            color:           card.state === 'verified'
                              ? 'rgba(80,210,130,0.9)'
                              : 'rgba(201,168,76,0.85)',
                          }}
                        >
                          {card.state === 'verified' && card.completion
                            ? `+${card.completion.xp_awarded}`
                            : card.xp_reward} XP
                        </span>
                      </div>
                      ...
                      {card.gold_reward > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                          ...
                          >
                            {card.state === 'verified' && card.completion
                              ? `+${card.completion.gold_awarded}`
                              : card.gold_reward} GP
                          </span>
                        </div>
                      )}

                      {/* Daily reset timer (only when this quest is completed) */}
                      {showTimer && (
                        <span
                          style={{
                            marginLeft:     'auto',
                    ...
                    {card.state === 'available' && (
                      <button
                        ...
                        style={{
                          border:      `1px solid ${diff.border}`,
                          background:  `linear-gradient(160deg, rgba(22,15,5,0.98), rgba(14,9,2,0.98))`,
                          color:        diff.color,
                          boxShadow:   `0 0 10px ${diff.color}20`,
                          '--btn-glow': `0 0 18px ${diff.color}35`,
                    ...
                    {card.state === 'verified' && (
                      <div
                        style={{
                          padding:        '0.85rem 1rem',
                          borderRadius:    2,
                          border:         '1px solid rgba(46,184,92,0.22)',
                          background:     'rgba(46,184,92,0.04)',
                          textAlign:      'center',
                          fontFamily:     'var(--font-pixel), monospace',
                          fontSize:       '0.42rem',
                          letterSpacing:  '0.1em',
                          color:          'rgba(46,184,92,0.7)',
                          imageRendering: 'pixelated',
                        }}
                      >
