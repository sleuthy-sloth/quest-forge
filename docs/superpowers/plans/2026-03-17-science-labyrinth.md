# Science Labyrinth Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a science mini-game for Quest Forge Academy where players navigate a first-person dungeon maze by answering multiple-choice science questions — correct answers advance through a corridor, wrong answers show a stone dead-end wall with an explanation.

**Architecture:** Two new files following the established WordForge/MathArena pattern exactly: a `'use client'` game component (`ScienceLabyrinth.tsx`) containing all game logic and UI, and a server route wrapper (`science-labyrinth/page.tsx`) that handles auth and profile fetch before passing props down. The academy catalog entry is already committed — no changes needed there.

**Tech Stack:** Next.js 14 App Router, TypeScript strict, Supabase JS client, React hooks, inline CSS keyframe animations, `AvatarPreview` component for avatar rendering.

**Spec:** `docs/superpowers/specs/2026-03-17-science-labyrinth-design.md`
**Reference implementations:** `src/components/games/WordForge.tsx`, `src/app/play/academy/math-arena/page.tsx`
...
| Action | File | Notes |
|---|---|---|
| Create | `src/app/play/academy/science-labyrinth/page.tsx` | Server component; copy of math-arena/page.tsx with component swapped |
| Create | `src/components/games/ScienceLabyrinth.tsx` | Full client game component |
| No change | `src/app/play/academy/page.tsx` | Catalog entry already present |

---

## Task 1: Route Wrapper

**Files:**
- Create: `src/app/play/academy/science-labyrinth/page.tsx`

- [ ] **Step 1: Create the route wrapper**

  Create `src/app/play/academy/science-labyrinth/page.tsx` with this exact content (mirrors `math-arena/page.tsx` — only the import and component name change):
...
- [ ] **Step 2: Type-check**
  ...
  ```
  Expected: error about `ScienceLabyrinth` module not found (the component doesn't exist yet). That error is expected — it confirms the route wrapper itself is syntactically correct and the import path matches where we'll put the component.

- [ ] **Step 3: Commit**

  ```bash
  git add src/app/play/academy/science-labyrinth/page.tsx
  git commit -m "feat(science-labyrinth): add route wrapper page"
  ```

---

## Task 2: Game Component — Scaffold, Types, Helpers, Loading Phase

**Files:**
- Create: `src/components/games/ScienceLabyrinth.tsx`

- [ ] **Step 1: Create the component with types, helpers, and loading/saving/results phases**
...
- [ ] **Step 2: Type-check**
  ...
  npx tsc --noEmit 2>&1 | head -20
  ```
  Expected: zero errors. If errors appear, read them carefully — most will be about the `supabase` client generic types or `profiles` column names. Fix before proceeding.

- [ ] **Step 3: Commit**

  ```bash
  git add src/components/games/ScienceLabyrinth.tsx src/app/play/academy/science-labyrinth/page.tsx
  git commit -m "feat(science-labyrinth): scaffold component with loading/saving/results phases"
  ```

---

## Task 3: Playing Phase — Arena Bar, Corridor Visual, Answer Handler, Question Card

**Files:**
- Modify: `src/components/games/ScienceLabyrinth.tsx` — replace the `return <div />` placeholder and add CSS keyframes + answer handler

- [ ] **Step 1: Replace the playing phase placeholder with the full implementation**

  In `ScienceLabyrinth.tsx`, replace:
  ```typescript
    // ── Playing phase (rendered below in Task 3) ───────────────────────────────
    return <div />
  }
  ...
  With the answer handler (insert before the `return <div />` line) and the full playing phase render. The complete replacement for everything from `// ── Playing phase` to the final `}` is:
...
- [ ] **Step 3: Lint**
  ...
  ```
  Expected: zero errors. Common lint issues to watch for: `no-unused-vars` on any import, missing `key` props.

- [ ] **Step 4: Commit**

  ```bash
  git add src/components/games/ScienceLabyrinth.tsx
  git commit -m "feat(science-labyrinth): complete playing phase — corridor visual, answer handler, dead-end wall"
  ```
...
No automated test framework is set up in this project. Verify manually:
...
- [ ] **Step 2: Navigate to the game**
  ...
  Open `http://localhost:3000/play/academy` in a browser. Confirm:
  - "Science Labyrinth" card appears in the academy grid
  - Clicking it navigates to `/play/academy/science-labyrinth`

- [ ] **Step 3: Verify loading state**

  On the game page, confirm "Charting the Labyrinth…" appears briefly while questions load. If the DB has no science challenges seeded, the "No challenges available" fallback should render with a back button.

- [ ] **Step 4: Verify correct answer flow**

  Answer a question correctly. Confirm:
  - Chosen button border turns green
  - Screen flashes green briefly (~300ms)
  - Corridor zooms forward and returns (~400ms)
  - Question advances after ~1 second
  - Score pip fills in the arena bar

- [ ] **Step 5: Verify wrong answer flow**

  Answer a question wrongly. Confirm:
  - Chosen button border turns red; correct button border turns green
  - Screen flashes red briefly (~300ms)
  - Stone wall drops over the corridor (~300ms)
  - Explanation text appears below the buttons (if the question has one)
  - After ~3 seconds: wall lifts, next question loads
  - After wall fully lifts (~300ms more): wall div is gone (cannot block clicks on next question)

- [ ] **Step 6: Verify last question + results**

  Answer all 10 questions. Confirm:
  - Saving screen shows "MAPPING YOUR ROUTE…" briefly
  - Results screen shows score, "LABYRINTH CLEARED" or "LABYRINTH ATTEMPTED", XP line, question log with "Path clear" / "Dead end" labels
  - "ENTER AGAIN" button resets and re-fetches a new set of questions
  - "← BACK TO ACADEMY" returns to `/play/academy`

- [ ] **Step 7: Commit verification**

  If any visual issues were found and fixed in the above steps, commit them now:

  ```bash
  git add -p
  git commit -m "fix(science-labyrinth): visual corrections from smoke test"
  ```
...
|---|---|
| 1 | Route wrapper |
| 2 | Component scaffold + loading/saving/results |
| 3 | Playing phase — corridor visual, answer handler, dead-end wall |
| 4 | Smoke test |
