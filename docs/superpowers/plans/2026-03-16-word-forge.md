# Word Forge Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a blacksmith-themed vocabulary game at `/play/academy/word-forge` that mirrors Math Arena's architecture with forge-specific visuals — anvil with heating iron bar, letter-glow animation on correct answers, blue accent throughout.

**Architecture:** Two files: a server component page that fetches profile data and a `'use client'` WordForge component that owns all game state. Questions are fetched from `edu_challenges` where `subject = 'vocabulary'`, shuffled client-side with Fisher-Yates, 10 taken. Same phase state machine as Math Arena: `loading → playing → saving → results`.

**Tech Stack:** Next.js 14 App Router, TypeScript, Supabase SSR (`@supabase/ssr`), Press Start 2P / Cinzel / Crimson Text fonts, AvatarPreview component, inline `<style>` for `iron-heat` and `letter-forge` keyframes, existing `card-rise` from globals.css.

---

## Chunk 1: Server Component Page

### Task 1: Create `src/app/play/academy/word-forge/page.tsx`

**Files:**
- Create: `src/app/play/academy/word-forge/page.tsx`
...
- [ ] **Step 2: Verify lint (WordForge import will error — expected)**

```bash
cd /home/spkoehl/claude/quest-forge && npm run lint 2>&1 | grep -v "WordForge" | tail -10
```

Expected: No errors unrelated to the missing WordForge component.

- [ ] **Step 3: Commit**

```bash
git add src/app/play/academy/word-forge/page.tsx
git commit -m "feat(academy): add word-forge server component page"
```

---

## Chunk 2: WordForge Client Component

### Task 2: Create `src/components/games/WordForge.tsx`

**Files:**
- Create: `src/components/games/WordForge.tsx`

- [ ] **Step 1: Create the complete WordForge component**
...
```bash
cd /home/spkoehl/claude/quest-forge && npm run type-check 2>&1 | tail -20
```
...
```bash
cd /home/spkoehl/claude/quest-forge && npm run lint 2>&1 | tail -20
```
...
```bash
cd /home/spkoehl/claude/quest-forge && npm run build 2>&1 | tail -20
```

Expected: `/play/academy/word-forge` route appears in build output.

- [ ] **Step 5: Commit**

```bash
git add src/components/games/WordForge.tsx
git commit -m "feat(academy): implement WordForge game — anvil theme, heat animations, vocab questions"
```
