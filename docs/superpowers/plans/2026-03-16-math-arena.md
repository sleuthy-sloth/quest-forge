# Math Arena Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a pixel-art battle math game at `/play/academy/math-arena` — 10 questions per session, training dummy opponent, correct/wrong animations, and DB saves for XP and boss damage.

**Architecture:** Two files: a server component page that fetches profile data and a `'use client'` MathArena component that owns all game state. Questions are fetched on mount (and on Play Again) via the browser Supabase client, shuffled client-side with Fisher-Yates, then 10 are taken. A single `phase` string drives the state machine: `loading → playing → saving → results`.

**Tech Stack:** Next.js 14 App Router, TypeScript, Supabase SSR, `@supabase/ssr`, Press Start 2P / Cinzel / Crimson Text fonts, AvatarPreview component, inline `<style>` for the `dummy-hit` keyframe, existing `card-rise` animation from globals.css.

---

## Chunk 1: Server Component Page

### Task 1: Create `src/app/play/academy/math-arena/page.tsx`

**Files:**
- Create: `src/app/play/academy/math-arena/page.tsx`
...
Expected: No errors related to this file (MathArena component doesn't exist yet so expect an import error — that's fine for now).

- [ ] **Step 3: Commit**

```bash
git add src/app/play/academy/math-arena/page.tsx
git commit -m "feat(academy): add math-arena server component page"
```

---

## Chunk 2: MathArena Client Component

### Task 2: Create `src/components/games/MathArena.tsx`

**Files:**
- Create: `src/components/games/MathArena.tsx`

- [ ] **Step 1: Write the complete MathArena component**
...
```bash
cd /home/spkoehl/claude/quest-forge && npm run type-check 2>&1 | tail -30
```
...
```bash
cd /home/spkoehl/claude/quest-forge && npm run build 2>&1 | tail -30
```
...
```bash
git add src/components/games/MathArena.tsx
git commit -m "feat(academy): implement MathArena game component with battle state machine"
```
